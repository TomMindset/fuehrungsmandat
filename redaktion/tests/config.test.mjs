...
test("Freigabe, Mail und bestätigte Produktionskanäle sind aktiv", async () => {
  ...
  assert.equal(config.publishing.live, true);
  for (const channel of ["website", "facebook", "instagram"]) {
    assert.equal(config.channels[channel].live, true);
  }
  assert.equal(config.channels.linkedin.live, false);
});
